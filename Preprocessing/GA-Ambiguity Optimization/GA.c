...
double calc_ambi(float **f, int r, int c) {
    double gL = 0.0;
	int i,j;
    double mean = 0.0;
    for (i = 0; i < r; i++) {
...
int main() {
    ...
    FILE *fp1 = fopen("low_truck.pgm", "r");
    FILE *fp2 = fopen("low_truck_GA1.pgm", "w");
    ...
    min = m;
 	for (i = 0; i < r; i++) 
	 for (j = 0; j < c; j++) 
		(in[i][j] > max) ? (max = in[i][j]) : ((in[i][j] < min) ? (min = in[i][j]) : 0);
